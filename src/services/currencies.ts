...
export const insertNewCurrencie = async (name:string , symbol:string, code:string) => {
...
export const getCurrencyById = async (id:number): Promise<Currency|false> => {
    try {
...
export const getCurrencyByName =  async (name:string): Promise<Currency|false>  => {
    try {
        const data = await db.query.currencies.findFirst({
...
export const removeCurrency = async (id:number) => {
    ...
    try {
        const removedCurrency  = await db.delete(currencies).where(eq(currencies.id, id)).returning();
...
export const editCurrency = async (cur:Currency) => {
    ...
    try {
        ...
        const editedCurrency  = await db.update(currencies).set({symbol, name, code}).where(eq(currencies.id, cur.id)).returning();
        if(!editedCurrency[0] ){
            return false;
        }
